...
my $referenceFile = '';
if(defined($::pack) && ($seqFile ne ''))
{
    ### FIXME/TODO This will need a full path to papa which will need
    ### to be installed during abYmod installation
    my $angle = `papa -q $seqFile`;
    chomp $angle;
    $referenceFile = FindReferenceFile($angle);

    print STDERR "\nUsing PAPA to predict packing angle...\n";
    print STDERR "   Predicted angle: $angle\n";
...
my $mod1 = FitChains($tmpDir, \%bestTemplates, $referenceFile);
print STDERR "done\n" if($::v >= 1);
...
#    if(open (my $fileHandle, '<:encoding(UTF-8)', $config::angleList))
...
#*************************************************************************
#> $model = FitChains($tmpDir, $hBestTemplates, $referenceFile)
#  -------------------------------------------------------------
...
#  29.11.16  Added $referenceFile paramater
...
sub FitChains
{
    ...
    else
    {
        ...
        # If a reference file is specified, add this information
	if($referenceFile ne '') 
        {
            $referenceFile = "$config::abpdblib/$referenceFile";
            $exe .= " -t $referenceFile";
            print STDERR "   Using PAPA-predicted packing angle\n" if($::v > 2);
	}
        else
        {
            print STDERR "   Using packing angle from $config::abpdblib/$light$config::pdbExt\n" if($::v > 2);
        }
...
#   V1.15  08.10.16  Added PAPA support and -pack option
...
# Using papa to find appropriate angle and template from sequence.
...
#
# This reads the template file that is generated byFF choosetemplates.pl
#
#  19.09.13 Original  By: ACRM
...
#*************************************************************************
#> $file = RunModeller($templatePDB, $seqFile, $tmpDir)
#  ----------------------------------------------------
...
abYmod V1.15 buildmodel (c) 2013-2016, Dr. Andrew C.R. Martin, UCL

Usage: ./buildmodel.pl [-k] [-v[=x]] [-force] [-cdr[=xx[,xx[...]]]] 
                       [-modeller] [-nomodeller] 
                       [-loopdb] [-noloopdb] [-loophit=nn] [-nloophits=nn]
                       [-nooptimize|-noopt] [-pack]
                       file.tpl [file.seq] > file.pdb
       -v[=x]      Verbose
       ...
       -pack       Use PAPA to predict the packing angle and fit chains to
                   a suitable angled template
